    ...
    >
      ...
      <div className='flex fixed z-10 flex-col border-2 w-4/5 gap-6 p-4'>
        ...
        <p className="text-center text-xl text-white tracking-wide">
          I&apos;m{' '}
          <span
            className={`hover:underline rainbow-text ${hoveredLink === 'https://www.linkedin.com/in/aichammmadi/' && 'hovered'}`}
            onMouseEnter={() => setHoveredLink('https://www.linkedin.com/in/aichammmadi/')}
            onMouseLeave={() => setHoveredLink(null)}
          >
            <a target="_blank" rel="noopener noreferrer" href="https://www.linkedin.com/in/aichammmadi/">
              Icham
            </a>
          </span>{' '}
          a 20 years old student in computer science, I&apos;m passionate about
          web development especially in the blockchain. I&apos;m currently working
          at{' '}
          <span
            className={`hover:underline rainbow-text ${hoveredLink === 'https://staky.io/' && 'hovered'}`}
            onMouseEnter={() => setHoveredLink('https://staky.io/')}
            onMouseLeave={() => setHoveredLink(null)}
          >
            <a target="_blank" rel="noopener noreferrer" href="https://staky.io/">
              Staky.Io
            </a>
          </span>{' '}
        ...
        <p className="text-center text-xl text-white tracking-wide">
          When I&apos;m not coding, I&apos;m on the football fields, I&apos;m currently playing
          for{' '}
          <span
            className={`hover:underline rainbow-text ${hoveredLink === 'https://www.afc-templiers.net/' && 'hovered'}`}
            onMouseEnter={() => setHoveredLink('https://www.afc-templiers.net/')}
            onMouseLeave={() => setHoveredLink(null)}
          >
            <a target="_blank" rel="noopener noreferrer" href="https://www.afc-templiers.net/">
              Les Templiers d&apos;Elancourt
            </a>
          </span>{' '}
          as Outclassed Player in the French National League (Defensive Back).I&apos;m also a big fan of the NBA and the NFL and
          obviously Video Games.
        </p>
      </div>
      {/* Affiche le GIF correspondant au lien survolé à la position de la souris */}
      {hoveredLink && mousePosition && (
        <div className="fixed z-30" style={{ top: mousePosition.y, left: mousePosition.x }}>
          <Image src={gifMappings[hoveredLink]} alt="GIF" width={200} height={200} />
        </div>
      )}
    </div>
  );
